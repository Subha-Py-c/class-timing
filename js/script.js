function getNextClass() {
  ...
  xhr.open("GET", "./jsons/schedule.json", true);
  xhr.send();
}

function findNextClass(schedule) {
  var current_time = new Date();
  var current_day = current_time
  ...
  if (current_day in schedule["time_table"]["days"]) {
    ...
    for (var i = 0; i < today_schedule.length; i++) {
      ...
      var class_start_time = new Date(
        current_time.toDateString() + " " + class_start_time_str
      );
      var class_end_time = new Date(
        current_time.toDateString() + " " + class_end_time_str
      );
